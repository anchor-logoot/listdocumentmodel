...
import { InternalError, FatalError } from './errors'

type LdmOptions = {
  /**
   * An option that will run tests on the DTST after every operation to it.
...
class OperationBuffer {
  ...
  constructor(
    protected readonly bst: DTst<AnchorLogootNode>,
  ...
  remove(node: AnchorLogootNode, start: number, length: number): void {
    ...
    if (successor) {
      successor.addSpaceBefore(-length, (np) => (this.bst.tst_root = np))
...
function fillSkipRanges(
  left: LogootPosition | undefined,
  right: LogootPosition | undefined,
  start: LogootPosition,
  clk: LogootInt,
  type: NodeType,
  skip_ranges: AnchorLogootNode[],
  ...
  return skip_ranges.flatMap((node, i) => {
    ...
    const space_avail = node.logoot_start.l(level)[0].copy().sub(last_level_pos)
      .js_int
    let nnode: AnchorLogootNode | undefined = undefined
    if (space_avail > 0) {
...
function linkFilledSkipRanges(
  ...
  let last_node_to_anchor: AnchorLogootNode | undefined = undefined
  filled_skip_ranges
  ...
  if (last_node_to_anchor && right) {
    // TS in strict mode complains that `Property 'reduceRight' does not exist
    // on type 'never'.` Not exactly sure whats up with that...
    ;(last_node_to_anchor as AnchorLogootNode).reduceRight(right)
  }
}
  ...
  capped_range.forEach((node: AnchorLogootNode, i) => {
    ...
    if (next) {
      ...
      scan_nodes.forEach((snode) => {
        const s_left: LeftAnchor | undefined = snode.true_left
        let n_right: RightAnchor | undefined = node.true_right
        ...
        if (
          ...
          if (
            ...
            let ctgt: AnchorLogootNode | undefined = snode.findLeftAnchorNode(
              node
            )
          ...
          if (
            ...
            if (n_right !== DocEnd && n_right.eq(snode.logoot_start)) {
              let itnode: AnchorLogootNode | undefined = snode
              ...
              if (itnode) {
                ...
                nodes_passed_over.forEach((n) =>
                  n.conflict_with.add(itnode as AnchorLogootNode)
                )
            ...
            let ctgt: AnchorLogootNode | undefined = node.findRightAnchorNode(
              next
            )
...
function insertTrailingConflicts(
  ...
  // Calculate the expected conflict. Returns the expected value and assigns to
  // the `expected` variable
  // TODO: Do assignment in while loop. This is confusing.
  let getExpectedValue = (): boolean => {
    const pos =
      true_left !== DocStart && (node as AnchorLogootNode).positionOf(true_left)
    if (pos && pos < (node as AnchorLogootNode).length) {
      node = (node as AnchorLogootNode).splitAround(pos)
      bstadd(node)
    }
    expected =
      true_left === DocStart ||
      ((node as AnchorLogootNode).logoot_end.gt(true_left) &&
        !true_left.equalsHigherLevel((node as AnchorLogootNode).logoot_end)) ||
      // Account for nodes with lower level ends:
      (node as AnchorLogootNode).logoot_end.equalsHigherLevel(true_left)
    return expected
  }
  ...
  getExpectedValue = (): boolean => {
    const pos =
      true_right !== DocEnd && (node as AnchorLogootNode).positionOf(true_right)
    if (pos && pos < (node as AnchorLogootNode).length) {
      bstadd((node as AnchorLogootNode).splitAround(pos))
    }
    expected =
      true_right === DocEnd ||
      (node as AnchorLogootNode).logoot_start.lt(true_right)
    return expected
  }
  ...
  skip_ranges.forEach((node) => {
    ...
    scan_nodes.forEach((snode) => {
      if (
        ...
        let cnode: AnchorLogootNode | undefined = node
        while (
          cnode &&
...
class ListDocumentModel {
   ...
   */
  bst: DTst<AnchorLogootNode> = new DTst()
  ...
  insertLocal(
    ...
    const search = (
      array: [number, AnchorLogootNode][]
    ): AnchorLogootNode | undefined => {
      let data_node: AnchorLogootNode | undefined = undefined
      array.forEach(([, node]) => {
        if (node.type === NodeType.DATA) {
          if (data_node) {
