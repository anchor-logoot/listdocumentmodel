...
class AnchorLogootNode extends DTstNode<AnchorLogootNode> {
    ...
    if (node.logoot_start.lt(this.logoot_start)) {
      ...
      if (node.true_right === 'E') {
        ...
        if (node.true_right.lt(this.logoot_end)) {
          // We already checked that the right position is within the start/end,
          // so there's no way this will return `undefined`
          const pos = this.positionOf(node.true_right) as number
      ...
      if (node.true_left === 'S') {
        ...
        if (node.true_left.gt(this.logoot_start)) {
          // Same as the comment on the above block
          const pos = this.positionOf(node.true_left) as number
          if (pos > 0 && pos < this.length) {
            nnode = this.splitAround(pos)
            cf_node = nnode
          }
  ...
  positionOf(pos: LogootPosition): number | undefined {
...
function sliceNodesIntoRanges(
  ...
  nodes.forEach((node: AnchorLogootNode | undefined) => {
